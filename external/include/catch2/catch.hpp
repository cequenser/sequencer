...
namespace Catch
{
    ...
    // Import begin/ end from std here so they are considered alongside the fallback (...) overloads
    // in this namespace
    using std::begin;
    using std::end;
...
namespace Catch
{
    ...
    // Specialised comparison functions to handle equality comparisons between ints and pointers
    // (NULL deduces as an int)
    template < typename LhsT, typename RhsT >
...
namespace Catch
{
    namespace Benchmark
    {
        namespace Detail
        {
            ...
            /// overhead
            /// in a measurement context needs to have consistent performance characteristics so
            /// that we
            /// can account for it in the measurement.
            /// Implementations of std::function with optimizations that aren't always applicable,
            /// like
            /// small buffer optimizations, are not uncommon.
            /// This is effectively an implementation of std::function without any such
            /// optimizations;
...
namespace Catch
{
    namespace Detail
    {
        ...
        {
            CATCH_ENFORCE( newMargin >= 0, "Invalid Approx::margin: "
                                               << newMargin << '.'
        ...
        namespace Floating
        {
                ...
                : m_target{target}, m_ulps{ulps}, m_type{baseType}
            {
                CATCH_ENFORCE( ulps >= 0, "Invalid ULP setting: "
                                              << ulps << '.' << " ULPs have to be non-negative." );
            }
