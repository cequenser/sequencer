...
#define TAINT_UNALIGNED( p, flg ) TAINT( p, ( (flg)&FFTW_UNALIGNED ) != 0 )
    ...
    tensor* X( mktensor_rowmajor )( int rnk, const int* n, const int* niphys, const int* nophys,
                                    int is, int os );
    ...
    const int* X( rdft2_pad )( int rnk, const int* n, const int* nembed, int inplace, int cmplx,
                               int** nfree );
