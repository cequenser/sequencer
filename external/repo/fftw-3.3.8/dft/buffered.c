...
{
    const P* ego = (const P*)ego_;
    p->print( p, "(dft-buffered-%D%v/%D-%D%(%p%)%(%p%)%(%p%))", ego->n, ego->nbuf, ego->vl,
   ...
   so as to allow optimizations in the the copy plan */
    ...
    cld = X( mkplan_f_d )( plnr,
                           X( mkproblem_dft_d )( X( mktensor_1d )( n, p->sz->dims[ 0 ].is, 2 ),
                                                 ...
                                                 TAINT( p->ri, ivs * nbuf ),
                                                 TAINT( p->ii, ivs * nbuf ), bufs + roffset,
                                                 bufs + ioffset ),
    ...
    /* plan the leftover transforms (cldrest): */
    {
        ...
        cldrest = X( mkplan_d )(
            plnr, X( mkproblem_dft_d )( X( tensor_copy )( p->sz ),
                                        X( mktensor_1d )( vl % nbuf, ivs, ovs ), p->ri + id,
                                        p->ii + id, p->ro + od, p->io + od ) );
    }
    if ( !cldrest )
        goto nada;
