...
ct_solver* ( *X( mksolver_ct_hook ) )( size_t, INT, int, ct_mkinferior, ct_force_vrecursion ) = 0;
...
{
    const P* ego = (const P*)ego_;
    p->print( p, "(dft-ct-%s/%D%(%p%)%(%p%))", ego->super.apply == apply_dit ? "dit" : "dif",
...
int X( ct_applicable )( const ct_solver* ego, const problem* p_, planner* plnr )
{
    ...
    return ( 0 || ego->dec == DECDIF + TRANSPOSE || p->vecsz->rnk == 0 || !NO_VRECURSEP( plnr ) ||
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    case DECDIT:
    {
        cldw = ego->mkcldw( ego, r, m * d[ 0 ].os, m * d[ 0 ].os, m, d[ 0 ].os, v, ovs, ovs, 0, m,
                            p->ro, p->io, plnr );
        if ( !cldw )
            goto nada;

        cld = X( mkplan_d )( plnr, X( mkproblem_dft_d )(
    ...
    case DECDIF + TRANSPOSE:
    {
        ...
        cldw = ego->mkcldw( ego, r, m * d[ 0 ].is, cors, m, d[ 0 ].is, v, ivs, covs, 0, m, p->ri,
                            p->ii, plnr );
        if ( !cldw )
            goto nada;
        ...
        cld = X( mkplan_d )(
            plnr, X( mkproblem_dft_d )( X( mktensor_1d )( m, d[ 0 ].is, r * d[ 0 ].os ),
                                        X( mktensor_2d )( r, cors, d[ 0 ].os, v, covs, ovs ), p->ri,
                                        p->ii, p->ro, p->io ) );
        if ( !cld )
            goto nada;
