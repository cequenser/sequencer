...
static int applicable0( const S* ego, INT r, INT irs, INT ors, INT m, INT v, INT mcount )
{
    return ( 1 && v == 1 && irs == ors && mcount >= ego->batchsz && mcount % ego->batchsz == 0 &&
             r >= 64 && m >= r );
}

static int applicable( const S* ego, INT r, INT irs, INT ors, INT m, INT v, INT mcount,
                       const planner* plnr )
{
...
static void dobatch( const P* ego, INT mb, INT me, R* buf, R* rio, R* iio )
{
    ...
    X( cpy2d_pair_co )
    ( buf, buf + 1, rio + ms * mb, iio + ms * mb, me - mb, 2 * BATCHDIST( ego->r ), ms, ego->r, 2,
      ego->rs );
}
...
static void apply( const plan* ego_, R* rio, R* iio )
{
    ...
    R* buf = (R*)MALLOC( sizeof( R ) * 2 * BATCHDIST( ego->r ) * ego->batchsz, BUFFERS );
    INT m;
...
{
    const P* ego = (const P*)ego_;
    p->print( p, "(dftw-genericbuf/%D-%D-%D%(%p%))", ego->batchsz, ego->r, ego->m, ego->cld );
}
                     ...
                     INT ovs, INT mstart, INT mcount, R* rio, R* iio, planner* plnr )
{
    ...
    cld = X( mkplan_d )( plnr,
                         X( mkproblem_dft_d )( X( mktensor_1d )( r, 2, 2 ),
