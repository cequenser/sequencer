...
static void dobatch( const P* ego, R* ri, R* ii, R* ro, R* io, R* buf, INT batchsz )
{
    X( cpy2d_pair_ci )
    ( ri, ii, buf, buf + 1, ego->n, WS( ego->is, 1 ), WS( ego->bufstride, 1 ), batchsz, ego->ivs,
      2 );
    ...
    else
    {
        ...
        ego->k( buf, buf + 1, buf, buf + 1, ego->bufstride, ego->bufstride, batchsz, 2, 2 );
        X( cpy2d_pair_co )
        ( buf, buf + 1, ro, io, ego->n, WS( ego->bufstride, 1 ), WS( ego->os, 1 ), batchsz, 2,
          ego->ovs );
    }
}
...
static void apply_extra_iter( const plan* ego_, R* ri, R* ii, R* ro, R* io )
{
   ...
   iteration as a 2-vector with vector stride 0. */
    ...
    ego->k( ri + ( vl - 1 ) * ego->ivs, ii + ( vl - 1 ) * ego->ivs, ro + ( vl - 1 ) * ego->ovs,
            io + ( vl - 1 ) * ego->ovs, ego->is, ego->os, 1, 0, 0 );
}
...
static void print( const plan* ego_, printer* p )
{
    ...
    if ( ego->slv->bufferedp )
        p->print( p, "(dft-directbuf/%D-%D%v \"%s\")", compute_batchsize( d->sz ), d->sz, ego->vl,
                  d->nam );
    else
        p->print( p, "(dft-direct-%D%v \"%s\")", d->sz, ego->vl, d->nam );
}

static int applicable_buf( const solver* ego_, const problem* p_, const planner* plnr )
{
...
static int applicable( const solver* ego_, const problem* p_, const planner* plnr,
                       int* extra_iterp )
{
    const S* ego = (const S*)ego_;
    const problem_dft* p = (const problem_dft*)p_;
    ...
    return (
        ...
        &&
        ( ( *extra_iterp = 0, ( d->genus->okp( d, p->ri, p->ii, p->ro, p->io, p->sz->dims[ 0 ].is,
                                               p->sz->dims[ 0 ].os, vl, ivs, ovs, plnr ) ) ) ||
          ( *extra_iterp = 1, ( ( d->genus->okp( d, p->ri, p->ii, p->ro, p->io, p->sz->dims[ 0 ].is,
