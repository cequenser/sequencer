...
static void cdot( INT n, const E* x, const R* w, R* or0, R* oi0, R* or1, R* oi1 )
{
    INT i;
...
static void hartley( INT n, const R* xr, const R* xi, INT xs, E* o, R* pr, R* pi )
{
    INT i;
    E sr, si;
    o[ 0 ] = sr = xr[ 0 ];
...
static void apply( const plan* ego_, R* ri, R* ii, R* ro, R* io )
{
    ...
    for ( i = 1; i + i < n; ++i )
    {
        cdot( n, buf, W, ro + i * os, io + i * os, ro + ( n - i ) * os, io + ( n - i ) * os );
        W += n - 1;
    }
...
static void awake( plan* ego_, enum wakefulness wakefulness )
{
    ...
    X( twiddle_awake )( wakefulness, &ego->td, half_tw, ego->n, ego->n, ( ego->n - 1 ) / 2 );
}
...
static int applicable( const solver* ego, const problem* p_, const planner* plnr )
{
    ...
    return ( 1 && p->sz->rnk == 1 && p->vecsz->rnk == 0 && ( p->sz->dims[ 0 ].n % 2 ) == 1 &&
...
static plan* mkplan( const solver* ego, const problem* p_, planner* plnr )
{
    ...
    static const plan_adt padt = {X( dft_solve ), awake, print, X( plan_null_destroy )};
