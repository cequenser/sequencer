...
{
    const P* ego = (const P*)ego_;
    p->print( p, "(indirect-transpose%v%(%p%)%(%p%)%(%p%))", ego->vl, ego->cldtrans, ego->cld,
              ego->cldrest );
}
...
static int applicable0( const solver* ego_, const problem* p_, const planner* plnr, int* pdim0,
                        int* pdim1 )
{
...
static int applicable( const solver* ego_, const problem* p_, const planner* plnr, int* pdim0,
                       int* pdim1 )
{
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    cldtrans =
        X( mkplan_d )( plnr, X( mkproblem_dft_d )( X( mktensor_0d )(), X( tensor_append )( tv, ts ),
                                                   rit, iit, rot, iot ) );
