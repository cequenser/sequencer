 ...
 * -reload-twiddle -dif -n 2 -name q1_2 -include dft/scalar/q.h */
...
static void q1_2( R* rio, R* iio, const R* W, stride rs, stride vs, INT mb, INT me, INT ms )
{
    {
        INT m;
        for ( m = mb, W = W + ( mb * 2 ); m < me; m = m + 1, rio = rio + ms, iio = iio + ms,
              W = W + 2, MAKE_VOLATILE_STRIDE( 4, rs ), MAKE_VOLATILE_STRIDE( 0, vs ) )
        {
