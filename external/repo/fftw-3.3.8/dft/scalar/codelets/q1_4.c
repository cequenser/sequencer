 ...
 * -reload-twiddle -dif -n 4 -name q1_4 -include dft/scalar/q.h */
...
static void q1_4( R* rio, R* iio, const R* W, stride rs, stride vs, INT mb, INT me, INT ms )
{
    {
        INT m;
        for ( m = mb, W = W + ( mb * 6 ); m < me; m = m + 1, rio = rio + ms, iio = iio + ms,
              W = W + 6, MAKE_VOLATILE_STRIDE( 8, rs ), MAKE_VOLATILE_STRIDE( 0, vs ) )
        {
