 ...
 * -pipeline-latency 8 -n 2 -name t1buv_2 -include dft/simd/t1bu.h -sign 1 */
...
{
    {
        ...
        R* x;
        x = ii;
        for ( m = mb, W = W + ( mb * ( ( TWVL / VL ) * 2 ) ); m < me;
              m = m + VL, x = x + ( VL * ms ), W = W + ( TWVL * 2 ), MAKE_VOLATILE_STRIDE( 2, rs ) )
        {
            V T1, T3, T2;
            T1 = LD( &( x[ 0 ] ), ms, &( x[ 0 ] ) );
 ...
 * 8 -n 2 -name t1buv_2 -include dft/simd/t1bu.h -sign 1 */
