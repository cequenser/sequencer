 ...
 * -pipeline-latency 8 -twiddle-log3 -precompute-twiddles -n 16 -name t2sv_16 -include dft/simd/ts.h
 */
...
static void t2sv_16( R* ri, R* ii, const R* W, stride rs, INT mb, INT me, INT ms )
{
    ...
    {
        INT m;
        for ( m = mb, W = W + ( mb * 8 ); m < me; m = m + ( 2 * VL ), ri = ri + ( ( 2 * VL ) * ms ),
              ii = ii + ( ( 2 * VL ) * ms ), W = W + ( ( 2 * VL ) * 8 ),
...
static const ct_desc desc = {16, XSIMD_STRING( "t2sv_16" ), twinstr, &GENUS, {104, 42, 92, 0}, 0, 0,
                             0};
 ...
 * -twiddle-log3 -precompute-twiddles -n 16 -name t2sv_16 -include dft/simd/ts.h */
...
static const ct_desc desc = {16, XSIMD_STRING( "t2sv_16" ), twinstr, &GENUS, {156, 68, 40, 0}, 0, 0,
                             0};
