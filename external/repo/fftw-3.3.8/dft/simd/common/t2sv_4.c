 ...
 * -pipeline-latency 8 -twiddle-log3 -precompute-twiddles -n 4 -name t2sv_4 -include dft/simd/ts.h
 */
...
static void t2sv_4( R* ri, R* ii, const R* W, stride rs, INT mb, INT me, INT ms )
{
    {
        INT m;
        for ( m = mb, W = W + ( mb * 4 ); m < me; m = m + ( 2 * VL ), ri = ri + ( ( 2 * VL ) * ms ),
              ii = ii + ( ( 2 * VL ) * ms ), W = W + ( ( 2 * VL ) * 4 ),
...
static const tw_instr twinstr[] = {VTW( 0, 1 ), VTW( 0, 3 ), {TW_NEXT, ( 2 * VL ), 0}};
 ...
 * -twiddle-log3 -precompute-twiddles -n 4 -name t2sv_4 -include dft/simd/ts.h */
