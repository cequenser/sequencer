 ...
 * -pipeline-latency 8 -twiddle-log3 -precompute-twiddles -n 8 -name t2sv_8 -include dft/simd/ts.h
 */
...
static void t2sv_8( R* ri, R* ii, const R* W, stride rs, INT mb, INT me, INT ms )
{
    ...
    {
        INT m;
        for ( m = mb, W = W + ( mb * 6 ); m < me; m = m + ( 2 * VL ), ri = ri + ( ( 2 * VL ) * ms ),
              ii = ii + ( ( 2 * VL ) * ms ), W = W + ( ( 2 * VL ) * 6 ),
...
static const tw_instr twinstr[] = {VTW( 0, 1 ), VTW( 0, 3 ), VTW( 0, 7 ), {TW_NEXT, ( 2 * VL ), 0}};

static const ct_desc desc = {8, XSIMD_STRING( "t2sv_8" ), twinstr, &GENUS, {44, 20, 30, 0}, 0, 0,
                             0};
 ...
 * -twiddle-log3 -precompute-twiddles -n 8 -name t2sv_8 -include dft/simd/ts.h */
...
static const ct_desc desc = {8, XSIMD_STRING( "t2sv_8" ), twinstr, &GENUS, {56, 26, 18, 0}, 0, 0,
                             0};
