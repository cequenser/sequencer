 ...
 * -pipeline-latency 8 -twiddle-log3 -precompute-twiddles -no-generate-bytw -n 4 -name t3bv_4
...
{
    {
        ...
        for ( m = mb, W = W + ( mb * ( ( TWVL / VL ) * 4 ) ); m < me;
              m = m + VL, x = x + ( VL * ms ), W = W + ( TWVL * 4 ), MAKE_VOLATILE_STRIDE( 4, rs ) )
        {
            V T2, T3, T4;
            T2 = LDW( &( W[ 0 ] ) );
            T3 = LDW( &( W[ TWVL * 2 ] ) );
 ...
 * 8 -twiddle-log3 -precompute-twiddles -no-generate-bytw -n 4 -name t3bv_4 -include dft/simd/t3b.h
 * -sign 1 */
