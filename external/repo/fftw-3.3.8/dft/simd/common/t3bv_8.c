 ...
 * -pipeline-latency 8 -twiddle-log3 -precompute-twiddles -no-generate-bytw -n 8 -name t3bv_8
...
static void t3bv_8( R* ri, R* ii, const R* W, stride rs, INT mb, INT me, INT ms )
{
    DVK( KP707106781, +0.707106781186547524400844362104849039284835938 );
    {
        ...
        R* x;
        x = ii;
        for ( m = mb, W = W + ( mb * ( ( TWVL / VL ) * 6 ) ); m < me;
              m = m + VL, x = x + ( VL * ms ), W = W + ( TWVL * 6 ), MAKE_VOLATILE_STRIDE( 8, rs ) )
        {
...
static const tw_instr twinstr[] = {VTW( 0, 1 ), VTW( 0, 3 ), VTW( 0, 7 ), {TW_NEXT, VL, 0}};

static const ct_desc desc = {8, XSIMD_STRING( "t3bv_8" ), twinstr, &GENUS, {27, 22, 10, 0}, 0, 0,
                             0};
 ...
 * 8 -twiddle-log3 -precompute-twiddles -no-generate-bytw -n 8 -name t3bv_8 -include dft/simd/t3b.h
 * -sign 1 */
