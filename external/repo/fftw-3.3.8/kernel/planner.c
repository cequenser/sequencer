...
static int subsumes( const flags_t* a, unsigned slvndx_a, const flags_t* b )
{
    ...
    {
        return ( LEQ( a->l, b->l ) && a->timelimit_impatience <= b->timelimit_impatience );
    }
}
...
static solution* htab_lookup( hashtab* ht, const md5sig s, const flags_t* flagsp )
{
...
static void fill_slot( hashtab* ht, const md5sig s, const flags_t* flagsp, unsigned slvndx,
                       solution* slot )
{
    ++ht->insert;
    ++ht->nelem;
...
static void hinsert0( hashtab* ht, const md5sig s, const flags_t* flagsp, unsigned slvndx )
{
    solution* l;
    unsigned g, h = h1( ht, s ), d = h2( ht, s );
...
static void htab_insert( hashtab* ht, const md5sig s, const flags_t* flagsp, unsigned slvndx )
{
    unsigned g, h = h1( ht, s ), d = h2( ht, s );
    solution* first = 0;
...
static void hinsert( planner* ego, const md5sig s, const flags_t* flagsp, unsigned slvndx )
{
    htab_insert( BLISS( *flagsp ) ? &ego->htab_blessed : &ego->htab_unblessed, s, flagsp, slvndx );
}

static void invoke_hook( planner* ego, plan* pln, const problem* p, int optimalp )
{
    if ( ego->hook )
...
static double random_estimate( const planner* ego, const plan* pln, const problem* p )
{
    md5 m;
    X( md5begin )( &m );
...
static plan* invoke_solver( planner* ego, const problem* p, solver* s, const flags_t* nflags )
{
...
static plan* search0( planner* ego, const problem* p, unsigned* slvndx, const flags_t* flagsp )
{
    plan* best = 0;
...
static plan* search( planner* ego, const problem* p, unsigned* slvndx, flags_t* flagsp )
{
    ...
    static const unsigned relax_tab[] = {0, /* relax nothing */
                                         NO_VRECURSE, NO_FIXED_RADIX_LARGE_N, NO_SLOW, NO_UGLY};
...
#define CHECK_FOR_BOGOSITY                                                                         \
    if ( ( ego->bogosity_hook ? ( ego->wisdom_state = ego->bogosity_hook( ego->wisdom_state, p ) ) \
        ...
        if ( LIVEP( l ) )
        {
           ...
           salva me fons pietatis */
            p->print( p, "  (%s %d #x%x #x%x #x%x #x%M #x%M #x%M #x%M)\n", reg_nam, reg_id,
                      l->flags.l, l->flags.u, l->flags.timelimit_impatience, l->s[ 0 ], l->s[ 1 ],
                      l->s[ 2 ], l->s[ 3 ] );
        }
    }
    p->print( p, ")\n" );
...
static int imprt( planner* ego, scanner* sc )
{
    ...
    while ( 1 )
    {
        ...
        if ( !sc->scan( sc, "(%*s %d #x%x #x%x #x%x #x%M #x%M #x%M #x%M)", MAXNAM, buf, &reg_id, &l,
                        &u, &timelimit_impatience, sig + 0, sig + 1, sig + 2, sig + 3 ) )
            goto bad;
...
planner* X( mkplanner )( void )
{
    ...
    static const planner_adt padt = {register_solver, mkplan, forget, exprt, imprt};
...
plan* X( mkplan_f_d )( planner* ego, problem* p, unsigned l_set, unsigned u_set, unsigned u_reset )
{
