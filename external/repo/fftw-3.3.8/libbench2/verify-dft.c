...
static void cpy0( dotens2_closure* k_, int indxa, int ondxa, int indxb, int ondxb )
{
...
static void dft_apply( dofft_closure* k_, bench_complex* in, bench_complex* out )
{
   ...
   real elements when using split format */
    ...
    if ( k->k.recopy_input )
        cpy( ri, ii, totalsz_swap, totalscale, &c_re( in[ 0 ] ), &c_im( in[ 0 ] ), pckdsz_swap, 1 );
...
void verify_dft( bench_problem* p, int rounds, double tol, errors* e )
{
    ...
    e->s = 0.0;
    e->s = dmax( e->s, tf_shift( &k.k, 0, p->sz, n, vecn, p->sign, inA, inB, outA, outB, tmp,
