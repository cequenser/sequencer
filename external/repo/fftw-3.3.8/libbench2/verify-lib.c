...
static double aerror( C* a, C* b, int n )
{
    if ( n > 0 )
    {
        ...
        {
            e = dmax( e,
                      norm2( c_re( a[ i ] ) - c_re( b[ i ] ), c_im( a[ i ] ) - c_im( b[ i ] ) ) );
...

        {
            ...
            for ( i = 0; i < N; ++i )
                ovtpvt_err( "%8d %16.12f %16.12f   %16.12f %16.12f\n", i, (double)c_re( a[ i ] ),
                            (double)c_im( a[ i ] ), (double)c_re( b[ i ] ),
                            (double)c_im( b[ i ] ) );
        }
...
static double impulse0( dofft_closure* k, int n, int vecn, C* inA, C* inB, C* inC, C* outA, C* outB,
                ...
                C* outC, C* tmp, int rounds, double tol )
{
    ...
    e = dmax( e, impulse0( k, n, vecn, inA, inB, inC, outA, outB, outC, tmp, rounds, tol ) );
...
double tf_shift( dofft_closure* k, int realp, const bench_tensor* sz, int n, int vecn, double sign,
                 C* inA, C* inB, C* outA, C* outB, C* tmp, int rounds, double tol, int which_shift )
{
...
static int one_accuracy_test( dofft_closure* k, aconstrain constrain, int sign, int n, C* a, C* b,
                              double t[ 6 ] )
{
    double err[ 6 ];
