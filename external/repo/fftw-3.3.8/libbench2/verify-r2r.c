...
static void raphase_shift( R* b, R* a, int n, int nb, int na, int n0, int k0, trigfun t )
{
    int j, jb, ja;
...
static void impulse_response( int rnk, dim_stuff* d, R impulse_amp, R* A, int N )
{
    if ( rnk == 0 )
        A[ 0 ] = impulse_amp;
    else
    {
        ...
        for ( i = 0; i < d->n; ++i )
        {
            impulse_response( rnk - 1, d + 1, impulse_amp * d->ti( d->i0, d->k0 + i, d->n0 ),
                              A + i * N, N );
        }
    }
}
...
static double rlinear( int n, info* nfo, R* inA, R* inB, R* inC, R* outA, R* outB, R* outC, R* tmp,
...
static double rimpulse( dim_stuff* d, R impulse_amp, int n, int vecn, info* nfo, R* inA, R* inB,
                        R* inC, R* outA, R* outB, R* outC, R* tmp, int rounds, double tol )
{
    double e = 0.0;
    int N = n * vecn;
    int i;
    ...
    for ( i = 0; i < vecn; ++i )
    {
        ...
        impulse_response( nfo->probsz->rnk, d, impulse_amp * inA[ i * n ], outA + i * n, n );
    }
...
static double t_shift( int n, int vecn, info* nfo, R* inA, R* inB, R* outA, R* outB, R* tmp,
                       int rounds, double tol, dim_stuff* d )
{
    double e = 0.0;
    ...
    for ( dim = 0; dim < sz->rnk; ++dim )
    {
        ...
        for ( j = 0; j < rounds; ++j )
        {
            ...
            dofft( nfo, inB, outB );
            for ( i = 0; i < vecn; ++i )
                raphase_shift( tmp + i * n, outA + i * n, ncur, nb, na, d[ dim ].n0, d[ dim ].k0,
                               d[ dim ].ts );
...
void verify_r2r( bench_problem* p, int rounds, double tol, errors* e )
{
    ...
    for ( i = 0; i < p->sz->rnk; ++i )
    {
        ...
        if ( p->k[ i ] > R2R_DHT )
            n0 = 2 *
                 ( n0 + ( p->k[ i ] == R2R_REDFT00 ? -1 : ( p->k[ i ] == R2R_RODFT00 ? 1 : 0 ) ) );
    ...
    e->i = rimpulse( d, impulse_amp, n, vecn, &nfo, inA, inB, inC, outA, outB, outC, tmp, rounds,
                     tol );
...
void accuracy_r2r( bench_problem* p, int rounds, int impulse_rounds, double t[ 6 ] )
{
    dofft_r2r_closure k;
    int n, n0 = 1;
    C *a, *b;
