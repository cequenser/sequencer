...
static void cpyr0( dotens2_closure* k_, int indxa, int ondxa, int indxb, int ondxb )
{
...
static void cpyr( R* ra, const bench_tensor* sza, R* rb, const bench_tensor* szb )
{
    cpyr_closure k;
    k.k.apply = cpyr0;
    k.ra = ra;
...
static void cpyhc20( dotens2_closure* k_, int indxa, int ondxa, int indxb, int ondxb )
{
...
static void cpyhc2( R* ra, R* ia, const bench_tensor* sza, const bench_tensor* vecsza, int scalea,
...
static void icpyhc20( dotens2_closure* k_, int indxa, int ondxa, int indxb, int ondxb )
{
    cpyhc2_closure* k = (cpyhc2_closure*)k_;
    int i, n = k->n;
    int scalea = k->scalea;
...
static void icpyhc2( R* ra, R* ia, const bench_tensor* sza, const bench_tensor* vecsza, int scalea,
                     R* rb, R* ib, const bench_tensor* szb )
{
    cpyhc2_closure k;
    BENCH_ASSERT( sza->rnk <= 1 );
...
static void rdft2_apply( dofft_closure* k_, bench_complex* in, bench_complex* out )
{
    ...
    if ( BENCH_FINITE_RNK( p->sz->rnk ) && p->sz->rnk > 0 )
        n2 = ( n2 / p->sz->dims[ p->sz->rnk - 1 ].n ) * ( p->sz->dims[ p->sz->rnk - 1 ].n / 2 + 1 );
    ri = (bench_real*)p->in;
    ro = (bench_real*)p->out;
...
void verify_rdft2( bench_problem* p, int rounds, double tol, errors* e )
{
    ...
    e->i = impulse( &k.k, n, vecn, inA, inB, inC, outA, outB, outC, tmp, rounds, tol );
    e->l = linear( &k.k, 1, N, inA, inB, inC, outA, outB, outC, tmp, rounds, tol );

    e->s = 0.0;
    if ( p->sign < 0 )
        e->s = dmax( e->s, tf_shift( &k.k, 1, p->sz, n, vecn, p->sign, inA, inB, outA, outB, tmp,
    ...
    if ( !p->in_place && !p->destroy_input )
        preserves_input( &k.k, p->sign < 0 ? mkreal : mkhermitian1, N, inA, inB, outB, rounds );
...
void accuracy_rdft2( bench_problem* p, int rounds, int impulse_rounds, double t[ 6 ] )
{
    dofft_rdft2_closure k;
    int n;
    C *a, *b;
    ...
    accuracy_test( &k.k, p->sign < 0 ? mkreal : mkhermitian1, p->sign, n, a, b, rounds,
                   impulse_rounds, t );
    bench_free( b );
    bench_free( a );
}
