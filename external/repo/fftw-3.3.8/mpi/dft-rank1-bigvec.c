...
static void print( const plan* ego_, printer* p )
{
    ...
    const char descrip[][ 16 ] = {"contig", "discontig", "square-after", "square-middle",
                                  "square-before"};
    p->print( p, "(mpi-dft-rank1-bigvec/%s%s %(%p%) %(%p%) %(%p%))", descrip[ ego->rearrange ],
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    cld = X( mkplan_d )(
        plnr, X( mkproblem_dft_d )( X( mktensor_1d )( nx, vn * 2, vn * 2 ),
                                    X( mktensor_2d )( yb, vn * 2 * nx, vn * 2 * nx, vn, 2, 2 ), ro,
                                    io, ri, ii ) );
    if ( XM( any_true )( !cld, p->comm ) )
        goto nada;

    cldt_after =
        X( mkplan_d )( plnr, XM( mkproblem_transpose )( ny, nx, vn * 2, I, O, yblock,
