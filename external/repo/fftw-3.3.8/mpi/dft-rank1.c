                       ...
                       INT rblock[ 2 ], INT mblock[ 2 ] )
{
    ...
    return (
       ...
       data distribution may be slightly different (ugh!) */
        ...
        && ( *r = XM( choose_radix )( p->sz->dims[ 0 ], n_pes, p->flags, p->sign, rblock, mblock ) )
...
static void print( const plan* ego_, printer* p )
{
    ...
    p->print( p, "(mpi-dft-rank1/%D%s%s%(%p%)%(%p%)%(%p%))", ego->r,
              ...
              ego->preserve_input == 2 ? "/p" : "", ego->cld_ddft, ego->cld_dft, ego->cldt );
}
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
   ...
   as if they were ddft_first */
    ...
    O = p->O;
    cld_ddft = X( mkplan_d )(
        plnr, XM( mkproblem_dft_d )( sz, rp * p->vn, I, O, p->comm, p->sign, RANK1_BIGVEC_ONLY ) );
    if ( XM( any_true )( !cld_ddft, p->comm ) )
        goto nada;

    I = TAINT( ( ddft_first || !p->flags ) ? p->O : p->I, rp * p->vn * 2 );
    O = TAINT( ( preserve_input || ( ddft_first && p->flags ) ) ? p->O : p->I, rp * p->vn * 2 );
