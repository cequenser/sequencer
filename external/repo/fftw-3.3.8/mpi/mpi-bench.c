...
static const char* mkversion( void )
{
    return FFTW( version );
}
...
static void alloc_rnk( int rnk_ )
{
    ...
    if ( rnk == 0 )
        ...
        local_ni = (ptrdiff_t*)bench_malloc( sizeof( ptrdiff_t ) * rnk * ( 8 + n_pes * 4 ) );
...
static void setup_gather_scatter( void )
{
    ...
    MPI_Gather( local_ni, rnk, FFTW_MPI_PTRDIFF_T, all_local_ni, rnk, FFTW_MPI_PTRDIFF_T, 0,
                MPI_COMM_WORLD );
    MPI_Bcast( all_local_ni, rnk * n_pes, FFTW_MPI_PTRDIFF_T, 0, MPI_COMM_WORLD );
    MPI_Gather( local_starti, rnk, FFTW_MPI_PTRDIFF_T, all_local_starti, rnk, FFTW_MPI_PTRDIFF_T, 0,
                MPI_COMM_WORLD );
    MPI_Bcast( all_local_starti, rnk * n_pes, FFTW_MPI_PTRDIFF_T, 0, MPI_COMM_WORLD );

    MPI_Gather( local_no, rnk, FFTW_MPI_PTRDIFF_T, all_local_no, rnk, FFTW_MPI_PTRDIFF_T, 0,
                MPI_COMM_WORLD );
    MPI_Bcast( all_local_no, rnk * n_pes, FFTW_MPI_PTRDIFF_T, 0, MPI_COMM_WORLD );
    MPI_Gather( local_starto, rnk, FFTW_MPI_PTRDIFF_T, all_local_starto, rnk, FFTW_MPI_PTRDIFF_T, 0,
                MPI_COMM_WORLD );
...
static void copy_block_out( const bench_real* in, int rnk, ptrdiff_t* n, ptrdiff_t* start,
                            ptrdiff_t is, ptrdiff_t* os, ptrdiff_t vn, bench_real* out )
{
    ptrdiff_t i;
    if ( rnk == 0 )
    {
...
static void copy_block_in( bench_real* in, int rnk, ptrdiff_t* n, ptrdiff_t* start, ptrdiff_t is,
                           ptrdiff_t* os, ptrdiff_t vn, const bench_real* out )
{
    ptrdiff_t i;
    if ( rnk == 0 )
    {
...
static void do_scatter_in( bench_real* in )
{
    ...
    for ( i = 0; i < n_pes; ++i )
    {
        copy_block_in( ali, rnk, all_local_ni + i * rnk, all_local_starti + i * rnk, vn, istrides,
                       vn, in );
        ali += isend_cnt[ i ];
    }
    MPI_Scatterv( all_local_in, isend_cnt, isend_off, BENCH_MPI_TYPE, local_in, isend_cnt[ my_pe ],
                  BENCH_MPI_TYPE, 0, MPI_COMM_WORLD );
}
...
static void do_gather_out( bench_real* out )
{
    ...
    MPI_Gatherv( local_out, orecv_cnt[ my_pe ], BENCH_MPI_TYPE, all_local_out, orecv_cnt, orecv_off,
                 BENCH_MPI_TYPE, 0, MPI_COMM_WORLD );
    MPI_Bcast( all_local_out, oNtot, BENCH_MPI_TYPE, 0, MPI_COMM_WORLD );
    alo = all_local_out;
    for ( i = 0; i < n_pes; ++i )
    {
        copy_block_out( alo, rnk, all_local_no + i * rnk, all_local_starto + i * rnk, vn, ostrides,
                        vn, out );
        alo += orecv_cnt[ i ];
    }
}
...
static FFTW( plan ) mkplan_transpose_local( ptrdiff_t nx, ptrdiff_t ny, ptrdiff_t vn,
...
static int tensor_contiguousp( bench_tensor* t, int s )
{
    return ( t->dims[ t->rnk - 1 ].is == s &&
             ( ( tensor_rowmajorp( t ) && t->dims[ t->rnk - 1 ].is == t->dims[ t->rnk - 1 ].os ) ||
...
static FFTW( plan ) mkplan_complex( bench_problem* p, unsigned flags )
{
    ...
    if ( p->sz->rnk < 1 || p->split || !tensor_contiguousp( p->sz, vn ) ||
         ...
         ( p->vecsz->rnk == 1 && ( p->vecsz->dims[ 0 ].is != 1 || p->vecsz->dims[ 0 ].os != 1 ) ) )
        return 0;
    ...
    if ( rnk > 1 )
    {
        ...
        ntot = FFTW( mpi_local_size_many_transposed )(
            ...
            MPI_COMM_WORLD, &n, &start, &nT, &startT );
    ...
    {
        ntot = FFTW( mpi_local_size_many_1d )( total_ni[ 0 ], vn, MPI_COMM_WORLD, p->sign, flags,
                                               local_ni, local_starti, local_no, local_starto );
    }
    alloc_local( ntot * 2, p->in == p->out );

    pln = FFTW( mpi_plan_many_dft )( p->sz->rnk, total_ni, vn, FFTW_MPI_DEFAULT_BLOCK,
                                     FFTW_MPI_DEFAULT_BLOCK, (FFTW( complex )*)local_in,
                                     (FFTW( complex )*)local_out, MPI_COMM_WORLD, p->sign, flags );
    ...
    if ( rnk > 1 )
    {
        ...
        if ( flags & FFTW_MPI_TRANSPOSED_OUT )
            plan_unscramble_out = mkplan_transpose_local( local_no[ 1 ], p->sz->dims[ 0 ].n,
                                                          vn * nrest, local_out, local_out );
    }
...
static FFTW( plan ) mkplan_real( bench_problem* p, unsigned flags )
{
    ...
    if ( p->sz->rnk < 2 || p->split || !tensor_real_contiguousp( p->sz, p->sign, vn ) ||
         tensor_rowmajor_transposedp( p->sz ) || p->vecsz->rnk > 1 ||
         ( p->vecsz->rnk == 1 && ( p->vecsz->dims[ 0 ].is != 1 || p->vecsz->dims[ 0 ].os != 1 ) ) )
        return 0;
    ...
    if ( p->sign < 0 )
        pln = FFTW( mpi_plan_many_dft_r2c )( p->sz->rnk, total_ni, vn, FFTW_MPI_DEFAULT_BLOCK,
                                             FFTW_MPI_DEFAULT_BLOCK, local_in,
                                             (FFTW( complex )*)local_out, MPI_COMM_WORLD, flags );
    else
        pln = FFTW( mpi_plan_many_dft_c2r )( p->sz->rnk, total_ni, vn, FFTW_MPI_DEFAULT_BLOCK,
                                             FFTW_MPI_DEFAULT_BLOCK, (FFTW( complex )*)local_in,
...

    {
        ...
        for ( i = 2; i < rnk; ++i )
            nrest *= total_ni[ i ];
        if ( flags & FFTW_MPI_TRANSPOSED_IN )
            plan_scramble_in = mkplan_transpose_local( total_ni[ 0 ], local_ni[ 1 ], vn * nrest,
                                                       local_in, local_in );
        if ( flags & FFTW_MPI_TRANSPOSED_OUT )
            plan_unscramble_out = mkplan_transpose_local( local_no[ 1 ], total_ni[ 0 ], vn * nrest,
                                                          local_out, local_out );
    }
...
static FFTW( plan ) mkplan_transpose( bench_problem* p, unsigned flags )
{
    ...
    else if ( d[ iy ].is == d[ ix ].n * vn && d[ iy ].os == vn && d[ ix ].os == d[ iy ].n * vn &&
              d[ ix ].is == vn )
    {
        nx = d[ iy ].n;
        ny = d[ ix ].n;
    ...
    alloc_rnk( 2 );
    ntot = vn * FFTW( mpi_local_size_2d_transposed )( nx, ny, MPI_COMM_WORLD, &local_ni[ 0 ],
                                                      &local_starti[ 0 ], &local_no[ 0 ],
                                                      &local_starto[ 0 ] );
    local_ni[ 1 ] = ny;
    local_starti[ 1 ] = 0;
    local_no[ 1 ] = nx;
    ...
    pln =
        FFTW( mpi_plan_many_transpose )( nx, ny, vn, FFTW_MPI_DEFAULT_BLOCK, FFTW_MPI_DEFAULT_BLOCK,
                                         local_in, local_out, MPI_COMM_WORLD, flags );

    if ( flags & FFTW_MPI_TRANSPOSED_IN )
        plan_scramble_in = mkplan_transpose_local( local_ni[ 0 ], ny, vn, local_in, local_in );
    if ( flags & FFTW_MPI_TRANSPOSED_OUT )
        plan_unscramble_out = mkplan_transpose_local( nx, local_no[ 0 ], vn, local_out, local_out );
...
static FFTW( plan ) mkplan_r2r( bench_problem* p, unsigned flags )
{
    ...
    pln = FFTW( mpi_plan_many_r2r )( p->sz->rnk, total_ni, vn, FFTW_MPI_DEFAULT_BLOCK,
                                     FFTW_MPI_DEFAULT_BLOCK, local_in, local_out, MPI_COMM_WORLD, k,
                                     flags );
    bench_free( k );
