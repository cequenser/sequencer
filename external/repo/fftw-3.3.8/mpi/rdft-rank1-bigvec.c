...
{
    const P* ego = (const P*)ego_;
    const char descrip[][ 16 ] = {"contig", "discontig", "square-after", "square-middle",
                                  "square-before"};
    p->print( p, "(mpi-rdft-rank1-bigvec/%s%s %(%p%) %(%p%) %(%p%))", descrip[ ego->rearrange ],
              ego->preserve_input == 2 ? "/p" : "", ego->cldt_before, ego->cld, ego->cldt_after );
}
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    cldt_before = X( mkplan_d )( plnr, XM( mkproblem_transpose )( nx, ny, vn, I = p->I, O = p->O,
    ...
    cld = X( mkplan_d )(
        plnr, X( mkproblem_rdft_1_d )( X( mktensor_1d )( nx, vn, vn ),
                                       X( mktensor_2d )( yb, vn * nx, vn * nx, vn, 1, 1 ), O, I,
                                       p->kind[ 0 ] ) );
    if ( XM( any_true )( !cld, p->comm ) )
        goto nada;

    cldt_after =
        X( mkplan_d )( plnr, XM( mkproblem_transpose )( ny, nx, vn, I, O, yblock,
