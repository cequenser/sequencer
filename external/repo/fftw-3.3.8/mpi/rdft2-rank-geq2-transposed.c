...
static void print( const plan* ego_, printer* p )
{
    ...
    p->print( p, "(mpi-rdft2-rank-geq2-transposed%s%(%p%)%(%p%)%(%p%))",
              ego->preserve_input == 2 ? "/p" : "", ego->cld1, ego->cldt, ego->cld2 );
}
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
        ...
        nrest *= sz->dims[ i ].n;
    {
        ...
        cld1 = X( mkplan_d )(
            plnr, X( mkproblem_rdft2_d )( sz, X( mktensor_2d )( b, is, is, p->vn, ivs, ovs ), r0,
                                          r1, cr, ci, p->kind ) );
    ...
    b1 = p->sz->dims[ 1 ].b[ k2 ];
    if ( p->sz->rnk == 2 )
    { /* n1 dimension is cut in ~half */
        n1 = n1 / 2 + 1;
        b1 = b1 == p->sz->dims[ 1 ].n ? n1 : b1;
    }

    if ( p->kind == R2HC )
        cldt = X( mkplan_d )( plnr, XM( mkproblem_transpose )( p->sz->dims[ 0 ].n, n1, nrest * 2, I,
                                                               O, p->sz->dims[ 0 ].b[ IB ], b1,
                                                               p->comm, 0 ) );
    else
        cldt = X( mkplan_d )( plnr, XM( mkproblem_transpose )( n1, p->sz->dims[ 0 ].n, nrest * 2, I,
                                                               O, b1, p->sz->dims[ 0 ].b[ OB ],
                                                               p->comm, 0 ) );
...

    {
        ...
        cld2 = X( mkplan_d )(
            plnr,
            X( mkproblem_dft_d )( X( mktensor_1d )( p->sz->dims[ 0 ].n, nrest * 2, nrest * 2 ),
