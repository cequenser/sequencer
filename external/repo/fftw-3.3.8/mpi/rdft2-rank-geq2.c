...
static int applicable( const S* ego, const problem* p_, const planner* plnr )
{
    ...
    return ( 1 && p->sz->rnk > 1 && p->flags == 0 /* TRANSPOSED/SCRAMBLED_IN/OUT not supported */
             && ( !ego->preserve_input ||
...
{
    const P* ego = (const P*)ego_;
    p->print( p, "(mpi-rdft2-rank-geq2%s%(%p%)%(%p%))", ego->preserve_input == 2 ? "/p" : "",
              ego->cld1, ego->cld2 );
}
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    cld2 = X( mkplan_d )( plnr, XM( mkproblem_dft_d )( sz2, nrest * p->vn, I, O, p->comm,
