...
static void apply( const plan* ego_, R* I, R* O )
{
    ...
    if ( cld1 )
    {
        ...
        if ( ego->equal_blocks )
            ...
            MPI_Alltoallv( O, ego->send_block_sizes, ego->send_block_offsets, FFTW_MPI_TYPE, I,
                           ego->recv_block_sizes, ego->recv_block_offsets, FFTW_MPI_TYPE,
                           ego->comm );
    }
    else
    { /* TRANSPOSED_IN, no need to destroy input */
        /* transpose chunks globally */
        if ( ego->equal_blocks )
            MPI_Alltoall( I, ego->send_block_sizes[ 0 ], FFTW_MPI_TYPE, O,
                          ego->recv_block_sizes[ 0 ], FFTW_MPI_TYPE, ego->comm );
        else
            MPI_Alltoallv( I, ego->send_block_sizes, ego->send_block_offsets, FFTW_MPI_TYPE, O,
                           ego->recv_block_sizes, ego->recv_block_offsets, FFTW_MPI_TYPE,
                           ego->comm );
    ...
    if ( cld2 )
    {
        ...
        if ( cld2rest )
        { /* leftover from unequal block sizes */
...
static void print( const plan* ego_, printer* p )
{
    ...
    p->print( p, "(mpi-transpose-alltoall%s%(%p%)%(%p%)%(%p%)%(%p%))",
              ego->equal_blocks ? "/e" : "", ego->cld1, ego->cld2, ego->cld2rest, ego->cld3 );
}
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    { /* I is already transposed */
        if ( ego->copy_transposed_in )
        {
            cld1 = X( mkplan_f_d )(
                plnr,
                X( mkproblem_rdft_0_d )( X( mktensor_1d )( b * p->ny * vn, 1, 1 ), I = p->I, p->O ),
                0, 0, NO_SLOW );
    ...
    { /* transpose b x ny x vn -> ny x b x vn */
        cld1 = X( mkplan_f_d )( plnr,
                                X( mkproblem_rdft_0_d )( X( mktensor_3d )( b, p->ny * vn, vn, p->ny,
                                                                           vn, b * vn, vn, 1, 1 ),
                                                         I = p->I, p->O ),
                                0, 0, NO_SLOW );
        if ( XM( any_true )( !cld1, p->comm ) )
    ...
    if ( XM( any_true )( !XM( mkplans_posttranspose )( p, plnr, I, p->O, my_pe, &cld2, &cld2rest,
