...
static void transpose_chunks( int* sched, int n_pes, int my_pe, INT* sbs, INT* sbo, INT* rbs,
                              INT* rbo, MPI_Comm comm, R* I, R* O )
{
    if ( sched )
    {
        int i;
            ...
            for ( i = 0; i < n_pes; ++i )
            {
                ...
                else
                {
                    ...
                    MPI_Sendrecv( buf, (int)( sbs[ pe ] ), FFTW_MPI_TYPE, pe,
                                  ( my_pe * n_pes + pe ) & 0xffff, O + rbo[ pe ],
                                  (int)( rbs[ pe ] ), FFTW_MPI_TYPE, pe,
                                  ( pe * n_pes + my_pe ) & 0xffff, comm, &status );
                }
            }
...
static void apply( const plan* ego_, R* I, R* O )
{
    ...
    if ( cld1 )
    {
        ...
        transpose_chunks( ego->sched, ego->n_pes, ego->my_pe, ego->send_block_sizes,
                          ego->send_block_offsets, ego->recv_block_sizes, ego->recv_block_offsets,
                          ego->comm, O, I );
    }
...
static int applicable( const S* ego, const problem* p_, const planner* plnr )
{
   ...
   at least with LAM MPI on my machine. */
    return ( 1 && ( !ego->preserve_input || ( !NO_DESTROY_INPUTP( plnr ) && p->I != p->O ) ) &&
...
static void print( const plan* ego_, printer* p )
{
    ...
    p->print( p, "(mpi-transpose-pairwise%s%(%p%)%(%p%)%(%p%)%(%p%))",
              ego->preserve_input == 2 ? "/p" : "", ego->cld1, ego->cld2, ego->cld2rest,
              ego->cld3 );
}
...
/* make the plans to do the post-MPI transpositions (shared with
   transpose-alltoall) */
int XM( mkplans_posttranspose )( const problem_mpi_transpose* p, planner* plnr, R* I, R* O,
                                 int my_pe, plan** cld2, plan** cld2rest, plan** cld3,
                                 INT* rest_Ioff, INT* rest_Ooff )
{
    ...
    *rest_Ioff = *rest_Ooff = 0;
    ...
    if ( !( p->flags & TRANSPOSED_OUT ) && ( nxr == 0 || I != O ) )
    {
        ...
        *cld2 = X( mkplan_f_d )(
            plnr,
            X( mkproblem_rdft_0_d )( X( mktensor_3d )( nxb, bt * b, b, bt, b, nx, b, 1, 1 ), I, O ),
            0, 0, NO_SLOW );
        if ( !*cld2 )
            goto nada;
    ...
    else
    {
        *cld2 = X( mkplan_f_d )(
            plnr,
            X( mkproblem_rdft_0_d )( X( mktensor_4d )( nxb, bt * b * vn, bt * b * vn, bt, b * vn,
                                                       vn, b, vn, bt * vn, vn, 1, 1 ),
                                     I, O ),
            0, 0, NO_SLOW );
        if ( !*cld2 )
            goto nada;
        ...
        *cld2rest =
        ...
        if ( !( p->flags & TRANSPOSED_OUT ) )
        {
            *cld3 = X( mkplan_f_d )(
                plnr,
                X( mkproblem_rdft_0_d )(
                    X( mktensor_3d )( p->nx, bt * vn, vn, bt, vn, p->nx * vn, vn, 1, 1 ), O, O ),
                0, 0, NO_SLOW );
            if ( !*cld3 )
                goto nada;
        }
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    { /* b x ny x vn -> ny x b x vn */
        cld1 = X( mkplan_f_d )(
            plnr,
            X( mkproblem_rdft_0_d )(
                X( mktensor_3d )( b, p->ny * vn, vn, p->ny, vn, b * vn, vn, 1, 1 ), I, O ),
            0, 0, NO_SLOW );
    ...
    if ( XM( any_true )( !XM( mkplans_posttranspose )( p, plnr, I, O, my_pe, &cld2, &cld2rest,
                                                       &cld3, &rest_Ioff, &rest_Ooff ),
                         p->comm ) )
        goto nada;
    ...
    n_pes =
        (int)X( imax )( XM( num_blocks )( p->nx, p->block ), XM( num_blocks )( p->ny, p->tblock ) );
