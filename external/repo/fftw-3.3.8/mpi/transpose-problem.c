...
static void print( const problem* ego_, printer* p )
{
    ...
    p->print( p, "(mpi-transpose %d %d %d %D %D %D %D %D %d)", ego->I == ego->O,
              X( ialignment_of )( ego->I ), X( ialignment_of )( ego->O ), ego->vn, ego->nx, ego->ny,
              ego->block, ego->tblock, i );
}
...
static const problem_adt padt = {PROBLEM_MPI_TRANSPOSE, hash, zero, print, destroy};

problem* XM( mkproblem_transpose )( INT nx, INT ny, INT vn, R* I, R* O, INT block, INT tblock,
