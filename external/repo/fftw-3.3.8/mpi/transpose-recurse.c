...
static int enough_space( INT nx, INT ny, INT block, INT tblock, int r, int n_pes )
{
    int pe;
    int m = n_pes / r;
...
static int applicable( const S* ego, const problem* p_, const planner* plnr, int* r )
{
    ...
    return (
        ...
        ( !NO_SLOWP( plnr ) || ( p->nx * ( p->ny / n_pes ) * p->vn ) / n_pes <= SMALL_MESSAGE ) &&
...
{
    const P* ego = (const P*)ego_;
    p->print( p, "(mpi-transpose-recurse/%s/%d%s%(%p%)%(%p%)%(%p%))", ego->nam, ego->r,
              ego->preserve_input == 2 ? "/p" : "", ego->cld1, ego->cldtr, ego->cldtm );
}
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    {   /* combine cld1 with TRANSPOSED_IN permutation */
        /* b x m x r x bt x vn -> r x m x bt x b x vn */
        INT vn = p->vn;
        INT bt = p->tblock;
        cld1 = X( mkplan_f_d )(
            plnr,
            X( mkproblem_rdft_0_d )( X( mktensor_5d )( b, m * r * bt * vn, vn, m, r * bt * vn,
                                                       bt * b * vn, r, bt * vn, m * bt * b * vn, bt,
                                                       vn, b * vn, vn, 1, 1 ),
                                     I, O ),
            0, 0, NO_SLOW );
    }
    else
    { /* TRANSPOSED_IN permutation must be separate for in-place */
        /* b x (m x r) x bt x vn -> b x (r x m) x bt x vn */
        INT vn = p->vn * p->tblock;
        cld1 = X( mkplan_f_d )(
            plnr,
            X( mkproblem_rdft_0_d )(
                X( mktensor_4d )( m, r * vn, vn, r, vn, m * vn, vn, 1, 1, b, np * vn, np * vn ), I,
                O ),
            0, 0, NO_SLOW );
    }
    ...
    if ( b )
        cldtm = X( mkplan_d )(
            plnr, XM( mkproblem_transpose )( p->nx, b, p->vn, I, O, r * p->block, p->tblock, comm2,
...
static solver* mksolver( int preserve_input, int ( *radix )( int np ), const char* nam )
{
