...
{
    const P* ego = (const P*)ego_;
    p->print( p, "(rdft-buffered-%D%v/%D-%D%(%p%)%(%p%)%(%p%))", ego->n, ego->nbuf, ego->vl,
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    if ( hc2rp )
    {
        ...
        cldcpy = X( mkplan_d )( plnr,
                                X( mkproblem_rdft_0_d )( X( mktensor_2d )( nbuf, ivs, bufdist, n,
                                                                           p->sz->dims[ 0 ].is, 1 ),
    ...
    else
    {
        ...
        cldcpy = X( mkplan_d )( plnr,
                                X( mkproblem_rdft_0_d )( X( mktensor_2d )( nbuf, bufdist, ovs, n, 1,
                                                                           p->sz->dims[ 0 ].os ),
    ...
    /* plan the leftover transforms (cldrest): */
    {
        ...
        cldrest =
            X( mkplan_d )( plnr, X( mkproblem_rdft_d )( X( tensor_copy )( p->sz ),
