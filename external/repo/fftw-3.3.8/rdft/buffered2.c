...
{
    const P* ego = (const P*)ego_;
    p->print( p, "(rdft2-buffered-%D%v/%D-%D%(%p%)%(%p%)%(%p%))", ego->n, ego->nbuf, ego->vl,
              ego->bufdist % ego->n, ego->cld, ego->cldcpy, ego->cldrest );
}
...
static int applicable0( const S* ego, const problem* p_, const planner* plnr )
{
    ...
    )
    {
        ...
        /* if this solver is redundant, in the sense that a solver
           ...
           solver */
        if ( X( nbuf_redundant )( d[ 0 ].n, vl, ego->maxnbuf_ndx, maxnbufs, NELEM( maxnbufs ) ) )
            return 0;
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
   ...
   so as to allow optimizations in the the copy plan */
    ...
    if ( p->kind == R2HC )
    {
        ...
        cldrest = X( mkplan_d )(
            plnr, X( mkproblem_rdft2_d )( X( tensor_copy )( p->sz ),
                                          X( mktensor_1d )( vl % nbuf, ivs, ovs ), p->r0 + id,
                                          p->r1 + id, p->cr + od, p->ci + od, p->kind ) );
        if ( !cldrest )
            goto nada;
    ...
    else
    {
        ...
        cldrest = X( mkplan_d )(
            plnr, X( mkproblem_rdft2_d )( X( tensor_copy )( p->sz ),
                                          X( mktensor_1d )( vl % nbuf, ivs, ovs ), p->r0 + od,
                                          p->r1 + od, p->cr + id, p->ci + id, p->kind ) );
        if ( !cldrest )
            goto nada;
