...
static void print( const plan* ego_, printer* p )
{
    ...
    p->print( p, "(rdft2-ct-%s/%D%(%p%)%(%p%))",
              ( ego->super.apply == apply_dit || ego->super.apply == apply_dit_dft ) ? "dit"
                                                                                     : "dif",
...
static int hc2c_applicable( const hc2c_solver* ego, const problem* p_, planner* plnr )
{
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    {
    case R2HC:
        cldw = ego->mkcldw( ego, R2HC, r, m * d[ 0 ].os, m, d[ 0 ].os, v, ovs, p->cr, p->ci, plnr );
        if ( !cldw )
            goto nada;
        ...
        case HC2C_VIA_RDFT:
            cld = X( mkplan_d )(
                plnr,
                X( mkproblem_rdft_1_d )( X( mktensor_1d )( m, ( r / 2 ) * d[ 0 ].is, d[ 0 ].os ),
        ...
        case HC2C_VIA_DFT:
            cld = X( mkplan_d )(
                plnr, X( mkproblem_dft_d )(
                          ...
                          X( mktensor_2d )( r / 2, d[ 0 ].is, m * d[ 0 ].os, v, ivs, ovs ), p->r0,
                          p->r1, p->cr, p->ci ) );
            if ( !cld )
                goto nada;
    ...
    case HC2R:
        cldw = ego->mkcldw( ego, HC2R, r, m * d[ 0 ].is, m, d[ 0 ].is, v, ivs, p->cr, p->ci, plnr );
        if ( !cldw )
            goto nada;
        ...
        case HC2C_VIA_RDFT:
            cld = X( mkplan_d )(
                plnr,
                X( mkproblem_rdft_1_d )( X( mktensor_1d )( m, d[ 0 ].is, ( r / 2 ) * d[ 0 ].os ),
        ...
        case HC2C_VIA_DFT:
            cld = X( mkplan_d )(
                plnr, X( mkproblem_dft_d )(
                          ...
                          X( mktensor_2d )( r / 2, m * d[ 0 ].is, d[ 0 ].os, v, ivs, ovs ), p->ci,
                          p->cr, p->r1, p->r0 ) );
            if ( !cld )
                goto nada;
...
hc2c_solver* X( mksolver_hc2c )( size_t size, INT r, hc2c_kind hc2ckind, hc2c_mkinferior mkcldw )
{
