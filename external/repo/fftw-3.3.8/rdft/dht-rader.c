...
static R* mkomega( enum wakefulness wakefulness, plan* p_, INT n, INT npad, INT ginv )
{
...
static void awake( plan* ego_, enum wakefulness wakefulness )
{
    ...
    default:
        ...
        A( !ego->omega );
        ego->omega = mkomega( wakefulness, ego->cld_omega, ego->n, ego->npad, ego->ginv );
        break;
    }
}
...
static void print( const plan* ego_, printer* p )
{
    ...
    p->print( p, "(dht-rader-%D/%D%ois=%oos=%(%p%)", ego->n, ego->npad, ego->is, ego->os,
              ego->cld1 );
...
static int applicable( const solver* ego, const problem* p_, const planner* plnr )
{
    ...
    UNUSED( ego );
    return (
        1 && p->sz->rnk == 1 && p->vecsz->rnk == 0 && p->kind[ 0 ] == DHT &&
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    cld1 = X( mkplan_f_d )( plnr,
                            X( mkproblem_rdft_1_d )( X( mktensor_1d )( npad, 1, 1 ),
                                                     X( mktensor_1d )( 1, 0, 0 ), buf, buf, R2HC ),
                            NO_SLOW, 0, 0 );
    if ( !cld1 )
        goto nada;

    cldp = X( mkproblem_rdft_1_d )( X( mktensor_1d )( npad, 1, 1 ), X( mktensor_1d )( 1, 0, 0 ),
                                    buf, buf,
#if R2HC_ONLY_CONV
                                    R2HC
#else
