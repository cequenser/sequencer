...
static void apply_r2hc( const plan* ego_, R* I, R* O )
{
    ...
    ego->k( I, I + ego->rs0, O, O + ego->ioffset, ego->rs, ego->csr, ego->csi, ego->vl, ego->ivs,
            ego->ovs );
}
...
static void apply_hc2r( const plan* ego_, R* I, R* O )
{
    const P* ego = (const P*)ego_;
    ASSERT_ALIGNED_DOUBLE;
    ego->k( O, O + ego->rs0, I, I + ego->ioffset, ego->rs, ego->csr, ego->csi, ego->vl, ego->ivs,
            ego->ovs );
}
...
{
    X( cpy2d_ci )
    ( I, buf, ego->n, ego->rs0, WS( ego->bcsr /* hack */, 1 ), batchsz, ego->ivs, 1, 1 );

    if ( IABS( WS( ego->csr, 1 ) ) < IABS( ego->ovs ) )
    {
        /* transform directly to output */
        ego->k( buf, buf + WS( ego->bcsr /* hack */, 1 ), O, O + ego->ioffset, ego->brs, ego->csr,
                ego->csi, batchsz, 1, ego->ovs );
    }
    else
    {
        /* transform to buffer and copy back */
        ego->k( buf, buf + WS( ego->bcsr /* hack */, 1 ), buf, buf + ego->bioffset, ego->brs,
                ego->bcsr, ego->bcsi, batchsz, 1, 1 );
        X( cpy2d_co )
        ( buf, O, ego->n, WS( ego->bcsr, 1 ), WS( ego->csr, 1 ), batchsz, 1, ego->ovs, 1 );
    }
}
...
static void dobatch_hc2r( const P* ego, R* I, R* O, R* buf, INT batchsz )
{
    if ( IABS( WS( ego->csr, 1 ) ) < IABS( ego->ivs ) )
    {
        ...
        ego->k( buf, buf + WS( ego->bcsr /* hack */, 1 ), I, I + ego->ioffset, ego->brs, ego->csr,
                ego->csi, batchsz, ego->ivs, 1 );
    }
    else
    {
        /* copy into buffer and transform in place */
        X( cpy2d_ci )
        ( I, buf, ego->n, WS( ego->csr, 1 ), WS( ego->bcsr, 1 ), batchsz, ego->ivs, 1, 1 );
        ego->k( buf, buf + WS( ego->bcsr /* hack */, 1 ), buf, buf + ego->bioffset, ego->brs,
                ego->bcsr, ego->bcsi, batchsz, 1, 1 );
    }
    X( cpy2d_co )
    ( buf, O, ego->n, WS( ego->bcsr /* hack */, 1 ), ego->rs0, batchsz, 1, ego->ovs, 1 );
}

static void iterate( const P* ego, R* I, R* O,
                     void ( *dobatch )( const P* ego, R* I, R* O, R* buf, INT batchsz ) )
{
    R* buf;
    INT vl = ego->vl;
    INT n = ego->n;
...
static void print( const plan* ego_, printer* p )
{
    ...
    if ( ego->slv->bufferedp )
        ...
        p->print( p, "(rdft-%s-direct-r2c-%D%v \"%s\")", X( rdft_kind_str )( s->desc->genus->kind ),
                  ego->n, ego->vl, s->desc->nam );
}
...
static int applicable( const solver* ego_, const problem* p_ )
{
    ...
    return ( 1 && p->sz->rnk == 1 && p->vecsz->rnk <= 1 && p->sz->dims[ 0 ].n == desc->n &&
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    X( ops_madd2 )( pln->vl / ego->desc->genus->vl, &ego->desc->ops, &pln->super.super.ops );
