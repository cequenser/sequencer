...
static void apply( const plan* ego_, R* r0, R* r1, R* cr, R* ci )
{
    ...
    ego->k( r0, r1, cr, ci, ego->rs, ego->cs, ego->cs, ego->vl, ego->ivs, ego->ovs );
}
...
static void print( const plan* ego_, printer* p )
{
    ...
    p->print( p, "(rdft2-%s-direct-%D%v \"%s\")", X( rdft_kind_str )( s->desc->genus->kind ),
              s->desc->n, ego->vl, s->desc->nam );
}
