...
static void awake( plan* ego_, enum wakefulness wakefulness )
{
    ...
    X( twiddle_awake )
    ( wakefulness, &ego->td, ego->slv->desc->tw, ego->r * ego->m, ego->r, ( ego->m - 1 ) / 2 );
}
...
static void print( const plan* ego_, printer* p )
{
    ...
    if ( slv->bufferedp )
        p->print( p, "(hc2hc-directbuf/%D-%D/%D%v \"%s\"%(%p%)%(%p%))", batchsz, ego->r,
                  X( twiddle_length )( ego->r, e->tw ), ego->v, e->nam, ego->cld0, ego->cldm );
    else
...
static int applicable( const S* ego, rdft_kind kind, INT r, INT m, INT v, const planner* plnr )
{
...
static plan* mkcldw( const hc2hc_solver* ego_, rdft_kind kind, INT r, INT m, INT ms, INT v, INT vs,
                     INT mstart, INT mcount, R* IO, planner* plnr )
{
    ...
    cldm = X( mkplan_d )(
        plnr, X( mkproblem_rdft_1_d )( ( CLDMP( m, mstart, mcount ) ? X( mktensor_1d )( r, rs, rs )
                                                                    : X( mktensor_0d )() ),
                                       X( mktensor_0d )(), TAINT( IO + imid, vs ),
                                       TAINT( IO + imid, vs ), kind == R2HC ? R2HCII : HC2RIII ) );
    if ( !cldm )
        goto nada;
    ...
    X( ops_madd2 )( v * ( ( pln->me - pln->mb ) / e->genus->vl ), &e->ops, &pln->super.super.ops );
...
static void regone( planner* plnr, khc2hc codelet, const hc2hc_desc* desc, int bufferedp )
{
...
void X( regsolver_hc2hc_direct )( planner* plnr, khc2hc codelet, const hc2hc_desc* desc )
{
