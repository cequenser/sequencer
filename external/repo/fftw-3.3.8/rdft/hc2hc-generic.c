...
{
    return ( 1 && ( kind == R2HC || kind == HC2R ) && ( m % 2 ) && ( r % 2 ) && !NO_SLOWP( plnr ) );
}
...
static void print( const plan* ego_, printer* p )
{
    ...
    p->print( p, "(hc2hc-generic-%s-%D-%D%v%(%p%)%(%p%))",
              ego->super.apply == apply_dit ? "dit" : "dif", ego->r, ego->m, ego->vl, ego->cld0,
              ego->cld );
}

static plan* mkcldw( const hc2hc_solver* ego_, rdft_kind kind, INT r, INT m, INT s, INT vl, INT vs,
                     INT mstart, INT mcount, R* IO, planner* plnr )
{
    ...
    cld0 = X( mkplan_d )( plnr,
                          X( mkproblem_rdft_1_d )( mstart == 0 ? X( mktensor_1d )( r, m * s, m * s )
                                                               : X( mktensor_0d )(),
   ...
   (where 2 corresponds to the real and imaginary parts) ...
    ...
    cld =
        X( mkplan_d )( plnr, X( mkproblem_rdft_1_d )( X( mktensor_1d )( r, m * s, m * s ),
