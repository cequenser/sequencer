...
{
    const P* ego = (const P*)ego_;
    p->print( p, "(rdft-ct-%s/%D%(%p%)%(%p%))", ego->super.apply == apply_dit ? "dit" : "dif",
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    {
    case R2HC:
        cldw = ego->mkcldw( ego, R2HC, r, m, d[ 0 ].os, v, ovs, 0, ( m + 2 ) / 2, p->O, plnr );
        if ( !cldw )
            goto nada;
    ...
    case HC2R:
        cldw = ego->mkcldw( ego, HC2R, r, m, d[ 0 ].is, v, ivs, 0, ( m + 2 ) / 2, p->I, plnr );
        if ( !cldw )
            goto nada;
