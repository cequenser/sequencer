...
static problem* mkcld_before( const problem_rdft* p )
{
    return X( mkproblem_rdft_d )( X( tensor_copy_inplace )( p->sz, INPLACE_OS ),
                                  X( tensor_copy_inplace )( p->vecsz, INPLACE_OS ), p->O, p->O,
                                  p->kind );
}

static const ndrct_adt adt_before = {apply_before, mkcld_before, "rdft-indirect-before"};
...
static problem* mkcld_after( const problem_rdft* p )
{
    return X( mkproblem_rdft_d )( X( tensor_copy_inplace )( p->sz, INPLACE_IS ),
                                  X( tensor_copy_inplace )( p->vecsz, INPLACE_IS ), p->I, p->I,
                                  p->kind );
}

static const ndrct_adt adt_after = {apply_after, mkcld_after, "rdft-indirect-after"};
...
static int applicable0( const solver* ego_, const problem* p_, const planner* plnr )
{
    ...
    return ( 1 &&
             ...
             ( 0
               ...
               || ( p->I != p->O && ego->adt->apply == apply_after && !NO_DESTROY_INPUTP( plnr ) &&
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    cldcpy = X( mkplan_d )(
        plnr, X( mkproblem_rdft_0_d )( X( tensor_append )( p->vecsz, p->sz ), p->I, p->O ) );
    if ( !cldcpy )
        goto nada;
