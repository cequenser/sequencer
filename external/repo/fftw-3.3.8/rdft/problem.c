...
{
    static const char kstr[][ KSTR_LEN ] = {"r2hc",    "r2hc01",  "r2hc10",  "r2hc11",  "hc2r",
                                            "hc2r01",  "hc2r10",  "hc2r11",  "dht",     "redft00",
                                            "redft01", "redft10", "redft11", "rodft00", "rodft01",
                                            "rodft10", "rodft11"};
...
static void print( const problem* ego_, printer* p )
{
    ...
    int i;
    p->print( p, "(rdft %d %D %T %T", X( ialignment_of )( ego->I ), ( INT )( ego->O - ego->I ),
              ego->sz, ego->vecsz );
                              ...
                              const rdft_kind* kind )
{
    ...
    ego = (problem_rdft*)X( mkproblem )(
        sizeof( problem_rdft ) + sizeof( rdft_kind ) * ( rnk > 0 ? rnk - 1u : 0u ), &padt );
   ...
   transform kind into account (sigh) */
    ...
    for ( i = 0; i < rnk; ++i )
        if ( ego->sz->dims[ i ].n == 2 &&
             ( ego->kind[ i ] == REDFT00 || ego->kind[ i ] == DHT || ego->kind[ i ] == HC2R ) )
...
problem* X( mkproblem_rdft_d )( tensor* sz, tensor* vecsz, R* I, R* O, const rdft_kind* kind )
{
...
problem* X( mkproblem_rdft_1 )( const tensor* sz, const tensor* vecsz, R* I, R* O, rdft_kind kind )
{
    A( sz->rnk <= 1 );
    return X( mkproblem_rdft )( sz, vecsz, I, O, &kind );
}

problem* X( mkproblem_rdft_1_d )( tensor* sz, tensor* vecsz, R* I, R* O, rdft_kind kind )
{
    A( sz->rnk <= 1 );
...
{
    return X( mkproblem_rdft_d )( X( mktensor_0d )(), vecsz, I, O, (const rdft_kind*)0 );
}
