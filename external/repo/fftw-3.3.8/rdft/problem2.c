...
static void print( const problem* ego_, printer* p )
{
    ...
    p->print( p, "(rdft2 %d %d %T %T)", (int)( ego->cr == ego->r0 ), (int)( ego->kind ), ego->sz,
              ego->vecsz );
}
...
static void vrecur( const iodim* vdims, int vrnk, const iodim* dims, int rnk, R* I0, R* I1 )
{
    if ( vrnk == RNK_MINFTY )
        return;
    else if ( vrnk == 0 )
...
static void zero( const problem* ego_ )
{
    ...
    if ( R2HC_KINDP( ego->kind ) )
    {
        ...
        vrecur( ego->vecsz->dims, ego->vecsz->rnk, ego->sz->dims, ego->sz->rnk, UNTAINT( ego->r0 ),
                UNTAINT( ego->r1 ) );
    }
    else
    {
        ...
        if ( rnk > 0 ) /* ~half as many complex outputs */
            sz2->dims[ rnk - 1 ].n = X( rdft2_complex_n )( sz2->dims[ rnk - 1 ].n, ego->kind );
...
problem* X( mkproblem_rdft2 )( const tensor* sz, const tensor* vecsz, R* r0, R* r1, R* cr, R* ci,
                               rdft_kind kind )
{
    problem_rdft2* ego;
   ...
   tensors first */
    ...
    if ( sz->rnk > 1 )
    { /* have to compress rnk-1 dims separately, ugh */
...
problem* X( mkproblem_rdft2_d )( tensor* sz, tensor* vecsz, R* r0, R* r1, R* cr, R* ci,
                                 rdft_kind kind )
{
...
/* Same as X(mkproblem_rdft2_d), but with only one R pointer.
   Used by the API. */
problem* X( mkproblem_rdft2_d_3pointers )( tensor* sz, tensor* vecsz, R* r0, R* cr, R* ci,
                                           rdft_kind kind )
{
    problem* p;
    int rnk = sz->rnk;
