...
static void print( const plan* ego_, printer* p )
{
    ...
    p->print( p, "(rdft2-rank>=2/%d%(%p%)%(%p%))", s->spltrnk, ego->cldr, ego->cldc );
}
...
static int applicable0( const solver* ego_, const problem* p_, int* rp, const planner* plnr )
{
...
/* TODO: revise this. */
static int applicable( const solver* ego_, const problem* p_, const planner* plnr, int* rp )
{
    const S* ego = (const S*)ego_;
    ...
    if ( NO_UGLYP( plnr ) )
    {
        ...
        /* Heuristic: if the vector stride is greater than the transform
           ...
           vrank-geq1 plan). */
        if ( p->vecsz->rnk > 0 &&
             X( tensor_min_stride )( p->vecsz ) > X( rdft2_tensor_max_index )( p->sz, p->kind ) )
            return 0;
    }
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    cldr = X( mkplan_d )( plnr, X( mkproblem_rdft2_d )( X( tensor_copy )( sz2 ),
                                                        X( tensor_append )( p->vecsz, sz1 ), p->r0,
                                                        p->r1, p->cr, p->ci, p->kind ) );
    if ( !cldr )
        goto nada;

    if ( p->kind == R2HC )
        cldp =
            X( mkproblem_dft_d )( X( tensor_copy_inplace )( sz1, k ),
                                  X( tensor_append )( vecszi, sz2i ), p->cr, p->ci, p->cr, p->ci );
    else /* HC2R must swap re/im parts to get IDFT */
        cldp =
            X( mkproblem_dft_d )( X( tensor_copy_inplace )( sz1, k ),
                                  X( tensor_append )( vecszi, sz2i ), p->ci, p->cr, p->ci, p->cr );
