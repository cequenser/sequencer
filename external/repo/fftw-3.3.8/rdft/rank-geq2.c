...
static void print( const plan* ego_, printer* p )
{
    ...
    p->print( p, "(rdft-rank>=2/%d%(%p%)%(%p%))", s->spltrnk, ego->cld1, ego->cld2 );
}
...
static int applicable0( const solver* ego_, const problem* p_, int* rp )
{
    ...
    return ( 1 && FINITE_RNK( p->sz->rnk ) && FINITE_RNK( p->vecsz->rnk ) && p->sz->rnk >= 2 &&
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    cld1 = X( mkplan_d )( plnr, X( mkproblem_rdft_d )( X( tensor_copy )( sz2 ),
                                                       X( tensor_append )( p->vecsz, sz1 ), p->I,
                                                       p->O, p->kind + spltrnk ) );
    if ( !cld1 )
        goto nada;

    cld2 = X( mkplan_d )( plnr, X( mkproblem_rdft_d )( X( tensor_copy_inplace )( sz1, INPLACE_OS ),
                                                       X( tensor_append )( vecszi, sz2i ), p->O,
                                                       p->O, p->kind ) );
    if ( !cld2 )
        goto nada;
