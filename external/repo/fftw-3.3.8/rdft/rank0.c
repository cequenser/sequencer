...
static void copy( const iodim* d, int rnk, INT vl, R* I, R* O, cpy2d_func cpy2d )
{
    A( rnk >= 2 );
    if ( rnk == 2 )
 ...
 * the real work */
static void transpose( const iodim* d, int rnk, INT vl, R* I, transpose_func transpose2d )
{
    A( rnk >= 2 );
    if ( rnk == 2 )
...
static int applicable( const S* ego, const problem* p_ )
{
    ...
    P pln;
    return ( 1 && p->sz->rnk == 0 && FINITE_RNK( p->vecsz->rnk ) && fill_iodim( &pln, p ) &&
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    static const plan_adt padt = {X( rdft_solve ), X( null_awake ), print, X( plan_null_destroy )};
...
void X( rdft_rank0_register )( planner* p )
{
    ...
    static struct
    {
        ...
        {apply_ip_sq_tiledbuf, applicable_ip_sq_tiledbuf, "rdft-rank0-ip-sq-tiledbuf"},
    };
