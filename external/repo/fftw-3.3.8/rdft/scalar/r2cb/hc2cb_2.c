 ...
 * -sign 1 -n 2 -dif -name hc2cb_2 -include rdft/scalar/hc2cb.h */
...
static void hc2cb_2( R* Rp, R* Ip, R* Rm, R* Im, const R* W, stride rs, INT mb, INT me, INT ms )
{
    {
        INT m;
        for ( m = mb, W = W + ( ( mb - 1 ) * 2 ); m < me; m = m + 1, Rp = Rp + ms, Ip = Ip + ms,
              Rm = Rm - ms, Im = Im - ms, W = W + 2, MAKE_VOLATILE_STRIDE( 8, rs ) )
        {
            E T1, T2, T6, T3, T4, T9;
            T1 = Rp[ 0 ];
            T2 = Rm[ 0 ];
 ...
 * -n 2 -dif -name hc2cb_2 -include rdft/scalar/hc2cb.h */
