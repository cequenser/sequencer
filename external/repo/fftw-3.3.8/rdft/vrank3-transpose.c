...
{
    rdftapply apply;
    int ( *applicable )( const problem_rdft* p, planner* plnr, int dim0, int dim1, int dim2,
                         INT* nbuf );
...
static int applicable( const solver* ego_, const problem* p_, planner* plnr, int* dim0, int* dim1,
                       int* dim2, INT* nbuf )
{
    const S* ego = (const S*)ego_;
    const problem_rdft* p = (const problem_rdft*)p_;

    return ( 1 && p->I == p->O && p->sz->rnk == 0 && ( p->vecsz->rnk == 2 || p->vecsz->rnk == 3 )
...
static int applicable_gcd( const problem_rdft* p, planner* plnr, int dim0, int dim1, int dim2,
                           INT* nbuf )
{
    ...
    *nbuf = n * ( m / d ) * vl;
    return ( !NO_SLOWP( plnr ) /* FIXME: not really SLOW for large 1d ffts */
             && n != m && d > 1 &&
             Ntuple_transposable( p->vecsz->dims + dim0, p->vecsz->dims + dim1, vl, vs ) );
}
...
static int mkcldrn_gcd( const problem_rdft* p, planner* plnr, P* ego )
{
    ...
    {
        ego->cld1 = X( mkplan_d )(
            plnr, X( mkproblem_rdft_0_d )( X( mktensor_3d )( n, d * m * vl, m * vl, d, m * vl,
                                                             n * m * vl, m * vl, 1, 1 ),
                                           TAINT( p->I, num_el ), buf ) );
        if ( !ego->cld1 )
            goto nada;
        X( ops_madd )( d, &ego->cld1->ops, &ego->super.super.ops, &ego->super.super.ops );
        ego->super.super.ops.other += num_el * d * 2;
    }

    ego->cld2 = X( mkplan_d )(
        plnr,
        X( mkproblem_rdft_0_d )( X( mktensor_3d )( d, d * n * m * vl, n * m * vl, d, n * m * vl,
                                                   d * n * m * vl, n * m * vl, 1, 1 ),
                                 p->I, p->I ) );
    if ( !ego->cld2 )
        goto nada;
    ...
    if ( m > 1 )
    {
        ego->cld3 = X( mkplan_d )(
            plnr, X( mkproblem_rdft_0_d )(
                      X( mktensor_3d )( d * n, m * vl, vl, m, vl, d * n * vl, vl, 1, 1 ),
                      TAINT( p->I, num_el ), buf ) );
        if ( !ego->cld3 )
            goto nada;
...
static const transpose_adt adt_gcd = {apply_gcd, applicable_gcd, mkcldrn_gcd, "rdft-transpose-gcd"};
...
static int applicable_cut( const problem_rdft* p, planner* plnr, int dim0, int dim1, int dim2,
                           INT* nbuf )
{
    INT n = p->vecsz->dims[ dim0 ].n;
    INT m = p->vecsz->dims[ dim1 ].n;
...
static int mkcldrn_cut( const problem_rdft* p, planner* plnr, P* ego )
{
    ...
    {
        ego->cld1 =
            X( mkplan_d )( plnr, X( mkproblem_rdft_0_d )( X( mktensor_3d )( nc, m * vl, vl, m - mc,
                                                                            vl, nc * vl, vl, 1, 1 ),
                                                          p->I + mc * vl, buf ) );
        if ( !ego->cld1 )
            goto nada;
    ...
    ego->cld2 = X( mkplan_d )(
        plnr, X( mkproblem_rdft_0_d )(
                  X( mktensor_3d )( nc, mc * vl, vl, mc, vl, nc * vl, vl, 1, 1 ), p->I, p->I ) );
    if ( !ego->cld2 )
        goto nada;
    X( ops_add2 )( &ego->cld2->ops, &ego->super.super.ops );
...
static const transpose_adt adt_cut = {apply_cut, applicable_cut, mkcldrn_cut, "rdft-transpose-cut"};
...
static void transpose_toms513( R* a, INT nx, INT ny, INT N, char* move, INT move_size, R* buf )
{
    INT i, im, mn;
    R *b, *c, *d;
    INT ncount;
...
static int applicable_toms513( const problem_rdft* p, planner* plnr, int dim0, int dim1, int dim2,
                               INT* nbuf )
{
    INT n = p->vecsz->dims[ dim0 ].n;
    INT m = p->vecsz->dims[ dim1 ].n;
...
static const transpose_adt adt_toms513 = {apply_toms513, applicable_toms513, mkcldrn_toms513,
