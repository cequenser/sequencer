...
{
    P* ego = (P*)ego_;
    static const tw_instr reodft00e_tw[] = {{TW_COS, 1, 1}, {TW_SIN, 1, 1}, {TW_NEXT, 1, 0}};

    X( plan_awake )( ego->clde, wakefulness );
    X( plan_awake )( ego->cldo, wakefulness );
    X( twiddle_awake )( wakefulness, &ego->td, reodft00e_tw, 2 * ego->n, 1, ego->n / 4 );
}
...
static void print( const plan* ego_, printer* p )
{
    ...
    if ( ego->super.apply == apply_e )
        p->print( p, "(redft00e-splitradix-%D%v%(%p%)%(%p%))", ego->n + 1, ego->vl, ego->clde,
                  ego->cldo );
    else
        p->print( p, "(rodft00e-splitradix-%D%v%(%p%)%(%p%))", ego->n - 1, ego->vl, ego->clde,
                  ego->cldo );
}
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    cldo = X( mkplan_d )( plnr, X( mkproblem_rdft_1_d )( X( mktensor_1d )( n / 2, 1, 1 ),
