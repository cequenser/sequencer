...
static void awake( plan* ego_, enum wakefulness wakefulness )
{
    ...
    X( twiddle_awake )( wakefulness, &ego->td, reodft010e_tw, 4 * ego->n, 1, ego->n / 2 + 1 );
    X( twiddle_awake )( wakefulness, &ego->td2, reodft11e_tw, 8 * ego->n, 1, ego->n * 2 );
}
...
{
    const P* ego = (const P*)ego_;
    p->print( p, "(%se-r2hc-%D%v%(%p%))", X( rdft_kind_str )( ego->kind ), ego->n, ego->vl,
              ego->cld );
}
