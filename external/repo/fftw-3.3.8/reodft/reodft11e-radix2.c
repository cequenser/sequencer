...
{
    P* ego = (P*)ego_;
    static const tw_instr reodft010e_tw[] = {{TW_COS, 0, 1}, {TW_SIN, 0, 1}, {TW_NEXT, 1, 0}};
    static const tw_instr reodft11e_tw[] = {{TW_COS, 1, 1}, {TW_SIN, 1, 1}, {TW_NEXT, 2, 0}};

    X( plan_awake )( ego->cld, wakefulness );

    X( twiddle_awake )( wakefulness, &ego->td, reodft010e_tw, 2 * ego->n, 1, ego->n / 4 + 1 );
...
{
    const P* ego = (const P*)ego_;
    p->print( p, "(%se-radix2-r2hc-%D%v%(%p%))", X( rdft_kind_str )( ego->kind ), ego->n, ego->vl,
              ego->cld );
}
...
static int applicable0( const solver* ego_, const problem* p_ )
{
    ...
    return ( 1 && p->sz->rnk == 1 && p->vecsz->rnk <= 1 && p->sz->dims[ 0 ].n % 2 == 0 &&
