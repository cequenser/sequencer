...
static void print( const plan* ego_, printer* p )
{
    const P* ego = (const P*)ego_;
    int i;
    p->print( p, "(dft-thr-ct-%s-x%d/%D", ego->super.apply == apply_dit ? "dit" : "dif", ego->nthr,
              ego->r );
    for ( i = 0; i < ego->nthr; ++i )
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    case DECDIT:
    {
        ...
        {
            cldws[ i ] = ego->mkcldw(
                ego, r, m * d[ 0 ].os, m * d[ 0 ].os, m, d[ 0 ].os, v, ovs, ovs, i * block_size,
                ( i == nthr - 1 ) ? ( m - i * block_size ) : block_size, p->ro, p->io, plnr );
            if ( !cldws[ i ] )
                goto nada;
        }
    ...
    case DECDIF + TRANSPOSE:
    {
        ...
        for ( i = 0; i < nthr; ++i )
        {
            cldws[ i ] = ego->mkcldw(
                ...
                ( i == nthr - 1 ) ? ( m - i * block_size ) : block_size, p->ri, p->ii, plnr );
            if ( !cldws[ i ] )
                goto nada;
        }
