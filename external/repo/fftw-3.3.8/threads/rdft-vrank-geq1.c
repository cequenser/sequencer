...
static void* spawn_apply( spawn_data* d )
{
    ...
    cld->apply( (plan*)cld, ego->I + thr_num * ego->its, ego->O + thr_num * ego->ots );
    return 0;
}
...
{
    return X( pickdim )( ego->vecloop_dim, ego->buddies, ego->nbuddies, vecsz, oop, dp );
}

static int applicable0( const solver* ego_, const problem* p_, const planner* plnr, int* dp )
{
    const S* ego = (const S*)ego_;
    const problem_rdft* p = (const problem_rdft*)p_;

    return ( 1 && plnr->nthr > 1 && FINITE_RNK( p->vecsz->rnk ) && p->vecsz->rnk > 0 &&
             pickdim( ego, p->vecsz, p->I != p->O, dp ) );
}

static int applicable( const solver* ego_, const problem* p_, const planner* plnr, int* dp )
{
    const S* ego = (const S*)ego_;
...
static plan* mkplan( const solver* ego_, const problem* p_, planner* plnr )
{
    ...
    for ( i = 0; i < nthr; ++i )
    {
        vecsz->dims[ vdim ].n = ( i == nthr - 1 ) ? ( d->n - i * block_size ) : block_size;
        cldp = X( mkproblem_rdft )( p->sz, vecsz, p->I + i * its, p->O + i * ots, p->kind );
