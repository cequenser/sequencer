...
static void add_problem( const char* pstring, bench_problem*** p, int* ip, int* np )
{
    if ( *ip >= *np )
    {
        *np = *np * 2 + 1;
...
static struct my_option options[] = {{"help", NOARG, 'h'},
                                     {"version", NOARG, 'V'},
                                     {"verbose", NOARG, 'v'},
...
static char canonical_sizes[][ 32 ] = {
    ...
    "2x2",        "4x4",     "8x8",     "10x10",    "16x16",     "32x32",     "64x64",
    "100x100",    "128x128", "256x256", "512x512",  "1000x1000", "1024x1024",

    "2x2x2",      "4x4x4",   "8x8x8",   "10x10x10", "16x16x16",  "32x32x32",  "64x64x64",
    "100x100x100"};
   ...
   user requests -T */
    ...
    while ( ( c = my_getopt( argc, argv, options ) ) != -1 )
    {
        ...
        case 'o':
            ...
            {
                output_fname = (char*)bench_malloc( sizeof( char ) * ( strlen( my_optarg ) + 1 ) );
    ...
    if ( canonical )
    {
        ...
        {
            unsigned j;
            char types[][ 8 ] = {"cof", "cob", "cif", "cib", "rof", "rob", "rif", "rib"};
