...
namespace sequencer::backend::synth
{
                          ...
                          const std::vector< std::pair< spectrum_t, spectrum_t > >& filters )
    {
        ...
        for ( auto j = 0; j < filters.size(); ++j )
        {
            const auto doff = int( j * spectrum_left.size() );
            auto g0 = std::async(
                std::launch::async, [&spectrum_left, &spectrum_right, j, doff, &filters]() mutable {
                    auto s = spectrum_left;
                    auto t = spectrum_right;
                    for ( auto k = 0u; k < s.size(); ++k )
                    {
                        s[ k ] *= 0.5f * filters[ j ].first[ k ];
                        t[ k ] *= 0.5f * filters[ j ].second[ k ];
                    }

                    return std::tuple{doff, audio::inverse_fft( s ), audio::inverse_fft( t )};
                } );
            done.push_back( std::move( g0 ) );
        }

        std::vector< float > tmp_left( spectrum_left.size() + filters.size() * spectrum_left.size(),
                                       0 );
    ...
    public:
                    ...
                    auto write_buffer = [&] {
                        ...
                        bool use_reverb = false;
                        {
                            ...
                            if ( !reverb_spectra_.empty() )
                            {
                                ...
                                std::tie( tmp_l, tmp_r ) = overlay_add_sub(
                                    spectrum0_f.get(), audio::fft( right ), reverb_spectra_ );
                                {
        ...
        void set_reverb_type_id( int id )
        {
            ...
            reverb_channels[ 0 ].resize( 16 * 1024 );
            reverb_channels[ 1 ].resize( 16 * 1024 );
