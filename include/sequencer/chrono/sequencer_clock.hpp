...
namespace sequencer::chrono
{
    inline namespace v1
    {
        ...
        public:
            ...
            using time_point = std::chrono::time_point< sequencer_clock< underlying_clock_type > >;
            ...
            time_point now() const noexcept
            {
                if ( is_running_ )
                {
                    return time_point{elapsed_ + underlying_clock_.now().time_since_epoch() -
            ...
            void start() noexcept
            {
                if ( !is_running_ )
                {
                    start_time_ = time_point{underlying_clock_.now().time_since_epoch()};
                    is_running_ = true;
                }
            }
            ...
            void stop() noexcept
            {
                if ( is_running_ )
                {
                    elapsed_ += duration{underlying_clock_.now().time_since_epoch() -
                                         start_time_.time_since_epoch()};
        ...
        private:
            ...
            time_point start_time_ = time_point{duration::zero()};
