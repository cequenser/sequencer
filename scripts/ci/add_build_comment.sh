...
sed 's/"/\\"/g' short_build.log | sed "s/'/\\\\\\\\'/g"  | sed 's@build/../include@include@g' | sed 's@build/../tests@tests@g' | sed "s@^../include@include@g" | sed "s@ ../include@ include@g" | sed "s@^../tests@tests@g" | sed "s@ ../tests@ tests@g" > tmp_comments.log 2>&1
