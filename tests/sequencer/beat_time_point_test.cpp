...
SCENARIO( "Construction", "[beat_time_point]" )
{
    GIVEN( "a default constructed beat_time_point" )
    {
        ...
        {
            STATIC_REQUIRE( t.time_since_epoch() == 0.0_beats );
        }
    }
    GIVEN( "a beat_duration of 3 beats" )
    {
        ...
        WHEN( "a beat_time_point is constructed" )
        {
            ...
            {
                STATIC_REQUIRE( t.time_since_epoch() == 3.0_beats );
            }
        }
    }
}
...
SCENARIO( "Addition", "[beat_time_point]" )
{
    ...
    GIVEN( "a beat_time_point of 16.0 since epoch" )
    {
        beat_time_point time_point( 16.0_beats );

        WHEN( "there is a duration of 13 beats" )
        {
            constexpr beat_duration duration{13.0};
            THEN( "in-place summation yields 29 beats" )
            {
                time_point += duration;
                REQUIRE( time_point.time_since_epoch() == 29.0_beats );
            }
        }
    }

    GIVEN( "a constexpr beat_time_point of 16.0 since epoch" )
    {
        constexpr beat_time_point time_point( 16.0_beats );
