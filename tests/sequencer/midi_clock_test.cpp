...
SCENARIO( "start and stop messages", "[midi_clock]" )
{
    ...
    GIVEN( "A midi_clock" )
    {
        ...
        WHEN( "the clock is not started" )
        {
            THEN( "nothing happens when update is called" )
            {
                clock.update( beat_time_point( 1.0_beats ),
                              []( midi_clock::message_type ) { throw 1; } );
            }
        ...
        WHEN( "the clock is started" )
        {
            ...
            {
                midi_clock::message_type message = midi_clock::message_type::invalid;
                clock.update(
                    beat_time_point( 0.0_beats ),
                    [& msg = message]( midi_clock::message_type message ) { msg = message; } );
                REQUIRE( message == midi_clock::message_type::start );
                ...
                THEN( "is_started returns false after stop" )
                {
                    ...
                    {
                        midi_clock::message_type received = midi_clock::message_type::invalid;
                        clock.update( beat_time_point( 1.0_beats ),
                                      [&received]( midi_clock::message_type message ) {
                                          REQUIRE( received == midi_clock::message_type::invalid );
                                          received = message;
                                      } );
                        REQUIRE( received == midi_clock::message_type::stop );
    ...
    GIVEN( "A midi_clock that starts at -4.0 beats" )
    {
        ...
        WHEN( "update is called at 1.0 beat " )
        {
            THEN( "the clock sends 5 x 24 pulses" )
            {
                int pulse_count = 0;
                clock.update( beat_time_point( 1.0_beats ),
                              [&pulse_count]( midi_clock::message_type message ) {
                                  if ( message == midi_clock::message_type::pulse )
                                  {
                                      pulse_count++;
                                  }
                              } );
...
SCENARIO( "continue", "[midi_clock]" )
{
    ...
    GIVEN( "a started midi_clock" )
    {
        midi_clock clock;
        clock.start();
        clock.update( beat_time_point( 0.0_beats ), []( midi_clock::message_type ) {} );
        ...
        WHEN( "the clock is stopped" )
        {
            ...
            THEN( "starting the clock again sends a continue message" )
            {
                ...
                REQUIRE( received == midi_clock::message_type::cont );
            }
        }
        ...
        WHEN( "the clock is reset" )
        {
            ...
            THEN( "starting the clock again sends a start message" )
            {
                ...
                REQUIRE( received == midi_clock::message_type::start );
            }
        }
    }
}
